...
num_img = len(glob.glob('sample images/*.dcm'))

start = 5   
num_img = 4 

fig, ax = plt.subplots(nrows=1, ncols=num_img, sharey=True, figsize=(num_img*10,10))
for q, file_path in enumerate(glob.glob('sample images/*.dcm')[start:start+num_img]):
    dataset = pydicom.dcmread(file_path)
    #show_dcm_info(dataset)
...
df = pd.read_csv('sample images/train-rle-sample.csv', header=None, index_col=0)
